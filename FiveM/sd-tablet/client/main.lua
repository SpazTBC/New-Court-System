...
-- Open court tablet
function OpenCourtTablet()
    if isTabletOpen then 
        if Config.Debug then
            print("Court tablet already open")
        end
        return 
    end
    
    if Config.Debug then
        print("Opening court tablet...")
    end
    ...
    SendNUIMessage({
        ...
        courtURL = Config.CourtURL
    })
    
    ShowNotification("Court tablet opened for " .. charData.firstName .. " " .. charData.lastName, "success")
end

-- Close court tablet
function CloseCourtTablet()
    if not isTabletOpen then return end
    
    if Config.Debug then
        print("Closing court tablet...")
    end
    ...
    ShowNotification("Court tablet closed", "primary")
end
...
-- Commands
RegisterCommand('courttablet', function()
    if Config.Debug then
        print("Court tablet command executed")
    end
    OpenCourtTablet()
end, false)

RegisterCommand('courttest', function()
    local charData = GetPlayerCharacterData()
    if charData then
...
AddEventHandler('onResourceStart', function(resourceName)
    if GetCurrentResourceName() == resourceName then
        if Config.Debug then
            print("Court tablet resource started")
            print("Framework:", FrameworkName or "None")
            print("Configured key:", Config.OpenKey, "Control ID:", GetKeyControl())
        end
        ShowNotification("Court tablet system loaded! Use /courttablet or " .. Config.OpenKey, "success")
    end
end)
