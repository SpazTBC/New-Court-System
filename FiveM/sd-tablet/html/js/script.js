...
let courtURL = 'http://localhost/courttablet/';
...
window.addEventListener('message', function(event) {
    ...
    switch(data.type) {
        case 'openTablet':
            openTablet(data.characterData, data.courtURL);
            break;
...
function loadCourtSystem(charData) {
    //console.log('Loading court system for:', charData.firstName, charData.lastName);
    
    const loadingText = document.getElementById('loadingText');
    if (loadingText) {
        loadingText.textContent = 'Loading court system for ' + charData.firstName + ' ' + charData.lastName + '...';
    }
    
    const characterName = charData.firstName + ' ' + charData.lastName;
    const url = courtURL + '?charactername=' + encodeURIComponent(characterName);
...
function goHome() {
    ...
    const homeUrl = courtURL + '?charactername=' + encodeURIComponent(characterName);
...
//console.log('Court tablet NUI script loaded');